	...
	public static void main(String[] args) {
		...
		while(true){
			try{
				...
				switch(input){
					case 1: 
						String query = "SELECT user.userId as Id , user.Name As Name , user.Email As Email , user.Phone As Phone "
								+ "FROM User "
								+ "WHERE user.usertype = 'Shopper' ;";
						driver.executeNewQuery(query);
						...
						driver.executeFirstQuery(userId);
						result = driver.getResult();
						printResult(result);
						break;
					case 2:
						String querySecond = "SELECT product.productId AS Id , product.productName As 'Product Name' "
								+ "FROM Product Order By(product.productId);";
						driver.executeNewQuery(querySecond);
						result = driver.getResult();
						while(result.next()){
						...
						if(driver.executeSecondQuery(urlArray,productId))
						System.out.println("Images Inserted");
						break;
					case 3:
						System.out.println(driver.executeThirdQuery()+" Rows Affected");
						break;
					case 4:
						driver.executeFourthQuery();
						result = driver.getResult();
