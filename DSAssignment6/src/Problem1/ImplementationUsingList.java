 ...
 * Adjacency List
 */
public class ImplementationUsingList implements UndrectedWeightedGraph {
	private List<LinkedList<AdjacencyList>> graph = new LinkedList<LinkedList<AdjacencyList>>();

	public ImplementationUsingList(List<LinkedList<AdjacencyList>> graph){
		if (graph == null) {
	...
	public List<Integer> reachable(int node) throws Exception {
		if (node >= graph.size() || node < 0) {
			throw new Exception("Given Node not Present in Graph");
		}
	...
	public int shortestPath(int node1, int node2) throws Exception {
		if (node1 >= graph.size() || node1 < 0 || node2 >= graph.size() || node2 < 0) {
			throw new Exception("Given Nodes not Present in Graph");
		}
